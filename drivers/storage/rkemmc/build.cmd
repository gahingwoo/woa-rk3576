@echo off
rem ---------------------------------------------------------------------------
rem Build rkemmc.sys for Windows on ARM64. Run from an EWDK build environment
...
msbuild rkemmc.vcxproj /p:Configuration=%CONFIG% /p:Platform=ARM64 /m
if errorlevel 1 (
    echo [!] build failed
    exit /b 1
)
...
echo.
echo [+] Output: ARM64\%CONFIG%\rkemmc\rkemmc.sys (and .inf/.cat)
endlocal
